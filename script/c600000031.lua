...
function s.initial_effect(c)
    -- Effetto di attivazione: Se un "Davide" è presente sul terreno, scarta un mostro Macchina per evocare un "Colelli"
...
-- Condizione: Controlla se un mostro con "Davide" nel nome è presente sul terreno
function s.condition(e,tp,eg,ep,ev,re,r,rp)
    return Duel.IsExistingMatchingCard(s.davidefilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
end
...
-- Target: Controlla se un mostro con "Colelli" nel nome può essere evocato specialmente
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(s.colellifilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp) end
