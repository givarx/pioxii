...
function c60000015.initial_effect(c)
    ...
    e1:SetCountLimit(1,60000015)
    e1:SetTarget(c60000015.thtg)
    e1:SetOperation(c60000015.thop)
    c:RegisterEffect(e1)
    ...
    e2:SetCondition(c60000015.tgcon)
    e2:SetValue(1)
    c:RegisterEffect(e2)
...
function c60000015.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c60000015.thfilter,tp,LOCATION_REMOVED,0,1,nil) end
    Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_REMOVED)
end
function c60000015.thop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local g=Duel.SelectMatchingCard(tp,c60000015.thfilter,tp,LOCATION_REMOVED,0,1,1,nil)
    if #g>0 then
